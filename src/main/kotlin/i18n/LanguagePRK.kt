...
class LanguagePRK : LanguageKOR() {
    ...
    override fun helpDev(): String = "주체자"
    override fun helpRepo(): String = "직결 저장소"
    override fun helpVersion(): String = "판올림"
    override fun helpSupport(): String = "련락 공간"

    override fun helpCmdInfo(): String = "GomokuBot / 시킴말"
    override fun helpCmdHelp(): String = "`~help` 시킴말을 알려 드리겠소."
    override fun helpCmdLang(langList: String): String =
        "`~lang` $langList 이 봉사기에서 사용되는 말씨 설정을 바꾼다우. Ex) `~lang` `PRK`"

    override fun helpCmdPVE(): String = "`~start` 콤퓨타와의 놀음을 시작합네다."
    override fun helpCmdPVP(): String = "`~start` `@언급` 언급된 놀음꾼과의 놀음을 시작 합네다. Ex) `~start` `@player`"
    override fun helpCmdResign(): String = "`~resign` 현재 진행하고 있는 놀음을 포기합네다."
    ...
    override fun gameAlreadyInProgress(nameTag: String): String = nameTag + "동무, 그곳에는 이미 돌이 놓여 있습네다. :thinking:"
    ...
    override fun gameCmdInfo(): String = " `~s` `미국말` `숫자` 형식으로 돌을 놓아주시우. Ex) `~s` `h` `8`"
    ...
    override fun gameFull(): String = "더이상 돌을 넣을 자리가 없어 무승부 처리 되었습네다."
