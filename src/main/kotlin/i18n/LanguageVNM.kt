...
class LanguageVNM : LanguageENG() {
    ...
    override fun helpDev(): String = "Nhà phát triển"
    override fun helpRepo(): String = "Xem mã nguồn trên GitHub"
    ...
    override fun helpCmdInfo(): String = "GomokuBot / Dòng lệnh"
    override fun helpCmdHelp(): String = "`~help` Xin trợ giúp"
    override fun helpCmdRank(): String = "`~rank` Hiện bảng xếp hạng TOP 10 người chơi xuất sắc"
    override fun helpCmdLang(langList: String): String =
        "`~lang` $langList Thay đổi ngôn ngữ khi giao tiếp với bot. VD) `~lang` `ENG`"
    override fun helpCmdSkin(): String = "`~skin` `A` `B` `C` Thay đổi skin của bot khi đánh. VD) `~skin` `A`"
    override fun helpCmdPVE(): String = "`~start` Đánh với máy."
    override fun helpCmdPVP(): String = "`~start` `@mention` Hai người chơi. VD) `~start` `@player`"
    override fun helpCmdResign(): String = "`~resign` Dừng ván đang chơi."

    override fun skinInfo(): String = "GomokuBot / Skin"
    ...
    override fun skinCmdInfo(style: String): String = "Nhập lệnh ``~skin`` ``$style`` để chọn skin này"
    ...
    override fun gameAlreadyInProgress(nameTag: String): String = "$nameTag, có người đã đánh chỗ đó rồi! :thinking:"
    ...
    override fun gameCmdInfo(): String = "Sử dụng lệnh  `~s` `alphabet` `number` để đánh. VD) `~s` `h` `8`"
    ...
    override fun gameFull(): String = "Ván này hoà vì hết ô để đánh."
