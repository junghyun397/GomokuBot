...
class LanguageSKO : LanguageKOR() {
    ...
    override fun helpDev(): String = "開發者"
    override fun helpRepo(): String = "Git 貯藏所"
    override fun helpVersion(): String = "版"
    override fun helpSupport(): String = "支援手段"

    override fun helpCmdInfo(): String = "GomokuBot / 命令語"
    override fun helpCmdHelp(): String = "`~help` 助言을告知해드립니다."
    override fun helpCmdLang(langList: String): String =
        "`~lang` $langList 此奉仕機에서使用되는言語設定을改變합니다. Ex) `~lang` `ENG`"
    override fun helpCmdPVE(): String = "`~start` 人工知能와의遊戱을開始합니다."
    override fun helpCmdPVP(): String = "`~start` `@言及` 言及된遊戱者와의遊戱을開始합니다. Ex) `~start` `@player`"
    override fun helpCmdResign(): String = "`~resign` 現在進行하고있는遊戱을抛棄합니다."
    ...
    override fun gameAlreadyInProgress(nameTag: String): String = "$nameTag 님, 彼地에는이미棋가置하여있습니다. :thinking:"
    ...
    override fun gameCmdInfo(): String = " `~s` `洋文` `數字`型式으로棋를置하여주세요. Ex) `~s` `h` `8`"
    ...
    override fun gameFull(): String = "더以上棋를置할位가無하여無勝負處理되었습니다."
