...
open class LanguageKOR : LanguageENG() {
    ...
    override fun helpDeveloper(): String = "개발자"
    override fun helpRepository(): String = "Git 저장소"
    override fun helpVersion(): String = "판올림"
    override fun helpSupport(): String = "지원 채널"
    override fun helpInvite(): String = "초대 링크"

    override fun helpCommandInfo(): String = "GomokuBot / 명령어"

    override fun helpCommandLang(langList: String): String =
        "`~lang` $langList 이 서버에서 사용되는 언어 설정을 바꿉니다. Ex) `~lang` `ENG`"
    override fun helpCommandSkin(): String = "`~skin` `A` `B` `C` 이 서버에서 사용되는 오목판의 스타일 설정을 바꿉니다. Ex) `~skin` `A`"
    override fun helpCommandPVE(): String = "`~start` 인공지능과의 게임을 시작합니다."
    override fun helpCommandPVP(): String = "`~start` `@언급` 언급된 플레이어와의 게임을 시작합니다. Ex) `~start` `@player`"
    override fun helpCommandResign(): String = "`~resign` 현재 진행되고 있는 게임을 포기합니다."

    override fun skinInformation(): String = "GomokuBot / 스타일"
    ...
    override fun skinCommandInfo(style: String): String = "이 스타일을 사용 하려면 ``~skin`` ``$style`` 명령어를 입력해주세요."
    ...
    override fun helpCommandRank(): String = "`~rank` 1위부터 10위까지의 순위를 출력합니다."
    ...
    override fun gameAlreadyExits(nameTag: String): String = "$nameTag 님, 그곳에는 이미 돌이 놓여 있습니다. :thinking:"
    ...
    override fun gameCommandInfo(): String = " `~s` `알파벳` `숫자` 형식으로 돌을 놓아주세요. Ex) `~s` `h` `8`"
    ...
    override fun gameTieCausedByFull(): String = "더는 돌을 놓을 자리가 없어 무승부 처리되었습니다."
