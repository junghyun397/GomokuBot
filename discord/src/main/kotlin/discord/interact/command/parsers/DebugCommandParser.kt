...
object DebugCommandParser : ParsableCommand {
    ...
    override fun parse(event: MessageReceivedEvent, languageContainer: LanguageContainer): Either<Command, ParseFailure> {
        ...
        val type = matchType(option)
            ?: return Either.Right(this.asParseFailure("unknown debug type") { _, _ -> IO { } })

        return Either.Left(DebugCommand("debug", type))
    }

    override fun parse(event: SlashCommandInteractionEvent, languageContainer: LanguageContainer) =
        Either.Right(this.asParseFailure("slash not supported") { _, _ -> IO {} })
